...
pub struct AdminSettleInvoice<'info> {
    #[account(
        mut,
        constraint = admin.key() == config.admin @ FactorizeError::Unauthorized
    )]
    ...
    pub usdc_admin_ata: InterfaceAccount<'info, TokenAccount>,
    #[account(
        mut,
        associated_token::mint = usdc_mint,
...
impl<'info> AdminSettleInvoice<'info> {
    pub fn admin_settle_invoice(
        &mut self,
        invoice_id: String,
        ...
        transfer_checked(
            CpiContext::new(
                ...
                TransferChecked {
                    from: self.usdc_admin_ata.to_account_info(),
